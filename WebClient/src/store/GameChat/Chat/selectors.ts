...
export type MessageInStoreWithUserName =
    | DefaultMessageWithUserName
    | AnnouncementMessage;

export const chatMessages =
    (chatType: ChatTypeEnum) => (state: AppState): MessageInStoreWithUserName[] | undefined => {
        ...
        const messages = state.gameChat.chats[chatType]?.messages.map(mess => {
            ...
            const authorWithName =
                state.rooms.currentRoom?.participantsWithNames.find(
