...
export const DurationPicker: React.FC<DurationPickerProps> = ({ value, onChange }) => {
    ...
    const handleMinutesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        onChange?.(
            dayjs()
                ...
                .set('second', +((matches && matches.length > 3 && matches[3]) || 0))
                .format('HH:mm:ss'),
        );
    };
    ...
    const handleSecondsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        onChange?.(
            dayjs()
                .set('hour', 0)
                .set('minute', +((matches && matches.length > 2 && matches[2]) || 0))
    ...
    return (
        <InputGroup>
            <Form.Control
                ...
                max="59"
                step="1"
                value={(matches && matches.length > 2 && matches[2]) || undefined}
            ...
            <Form.Control
                ...
                max="59"
                step="1"
                value={(matches && matches.length > 3 && matches[3]) || undefined}
