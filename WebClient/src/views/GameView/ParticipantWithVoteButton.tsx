...
export const ParticipantWithVoteButton: React.FC<ParticipantProps> = ({
    className,
    participant: user,
    phase,
}) => {
    ...
    const [shouldShowVoteButton, currentUserVote] = useMemo(() => {
        ...
        showButton =
            ...
            !user.roles.includes(RoleEnum.Ghost);

        if (currentUser.id === user.id) {
            showButton = false;
        }

        const currUserVote = voteState.find(
    ...
    const votesForUser = useMemo(
        () => currentGameState?.voteState
                ...
                .map(
                    vote => participants?.find(u => u.id === vote.userId)?.name,
                ),
    ...
    return (
        <Participant className={className}>
            <ParticipantName
                isHighlighted={
                    currentUserVote && currentUserVote.votedUserId === user.id
                }
            >
                {user.name}
            </ParticipantName>{' '}
